...
void setup() {
  ...
  fillTetrisScreen(0);
  attachInterrupt(1, myLEFT, RISING);//KEY_LEFT
  attachInterrupt(3, myRIGHT, RISING);//KEY_RIGHT
  attachInterrupt(2, myDOWN, RISING);//KEY_DOWN
  attachInterrupt(6, myROTATE, RISING);//KEY_ROTATE

  randomSeed(analogRead(17)); /// To do: create a decent random number generator.
}
